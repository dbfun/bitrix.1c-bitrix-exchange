...
В файле `bundle.sh` содержатся различные кейсы загрузки данных на рабочий сайт. Данные для подключения к сайту следует указать в `.env`-файле, а для кейса - в виде параметров вызова:

* `-c` - вариант обмена
* `-s` - исходный файл (опционально)
* `-f` - имя файла (опционально, будет передан как GET параметр)
* `-d` - DNS (опционально)
* `-e` - `.env`-файл (опционально, default: `.env`)

Пример:

```
./bundle.sh -c custom.csv -s ./data/zony.csv -f zony.csv -d 10.0.0.9
```

В ряде случаев использование бандла позволяет существенно сократить команду, что удобно:

```
./bundle.sh -c import.xml
```
